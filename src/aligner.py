...
class Aligner:
    ...
    def performAffine(self):
        ...
        ddfx = (
            Resize(spatial_size=size_target, mode="trilinear")(ddf_ds[:, 0])
            * (size_moving[0] / SZ)
        ).to("cpu")
        ddfy = (
            Resize(spatial_size=size_target, mode="trilinear")(ddf_ds[:, 1])
            * (size_moving[1] / SZ)
        ).to("cpu")
        ddfz = (
            Resize(spatial_size=size_target, mode="trilinear")(ddf_ds[:, 2])
            * (size_moving[2] / SZ)
        ).to("cpu")
...
def center_and_resample_images(
    sub_img, atlas_img, centered_atlas, atlas_labels=None, centered_atlas_labels=None
):
    """
...
def main():
    ...
    parser.add_argument(
        "-ddf", "--ddf-file", type=str, help="dense displacement field file name"
    )
    parser.add_argument(
        "--nn_input_size",
        type=int,
    ...
    parser.add_argument(
        "--lr", type=float, default=1e-6, help="learning rate (default: 1e-4)"
    )
    parser.add_argument(
        "-e", "--max-epochs", type=int, default=1500, help="maximum interations"
    )
    parser.add_argument(
        "-d", "--device", type=str, default="cuda", help="device: cuda, cpu, etc."
    )
    parser.add_argument(
        "-l", "--loss", type=str, default="mse", help="loss function: mse, cc or mi"
    )
